...
#' simulate true genotype-pairs from linked markers using Mendel
#'
#' @param df A data frame in the format of \code{\link{long_markers}}.
#' @param ped The pedigree to be simulating from.
#' @param num Number of reps of gene-dropping to do. Default is 1000.
#' @keywords internal
sample_linked_true_genotype_pairs_mendel <- function(df, ped, num = 1000) {
  ...
  run_mendel(tmpDir, "mendel-example-Control.in")
  read_mendel_outped(file.path(tmpDir, "mendel-example-Ped.out"), alle_nums$n, verbose = getOption("CKMRsim.linkage_verbosity", default = 1)) %>%
    lapply(function(x) {
