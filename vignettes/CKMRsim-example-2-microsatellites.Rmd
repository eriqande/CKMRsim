...
```{r}
ggplot(logls, aes(x = logl_ratio, fill = true_relat)) +
...
final <- next_step %>%
  mutate(matchers = pmap(.l= list(x = GaF, y = CK, z = 10), 
         ...
         FS_candidates = pmap(.l = list(x = GaF, y = CK), 
                              .f = function(x, y) pairwise_kin_logl_ratios(D1 = x$Genos, D2 = x$Genos, CK = y, numer = "FS", denom = "U", keep_top = 20) %>% arrange(desc(logl_ratio)) %>% slice(1:50))
         )
```
